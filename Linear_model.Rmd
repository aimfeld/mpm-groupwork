...
First we start with a simple model and take only one predictor and start with the variable revenue. We wanna predict if the revenue increases with a higher Peak CCU.  

```{r simple lm ccu}
df <- games[, c("Peak.CCU.log", "Revenue.log")]

# Creating a simple linear model
m <- lm(Peak.CCU.log ~ Revenue.log, data = df)

# Plot
plot(df$Revenue.log, df$Peak.CCU.log, pch = 20, col = 'lightblue',
...
Therefore we are now adjusting our linear model, so that we can increase the R-squared value and adding more predictors.  
