...
games <- games %>% select(
    -c(Estimated.owners, About.the.game, Reviews, Header.image, Full.audio.languages, Developers,
...
A few publishers produce many games, while most publishers only produce a few games. We will create a new column that contains the number of games produced by each publisher.
...
kable(head(publisher.counts, 10))
```

We will also create dummy variables for the top 10 genres and categories. The tags are somewhat redundant with the genres, so we will not use them.
