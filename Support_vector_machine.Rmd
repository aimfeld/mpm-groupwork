...
During the cross-validation, the model achieved an R-squared value of 0.79. On the test set, the R-squared is a bit lower (0.73), which is pretty good. Since we used the logarithm of revenue, we get a nice scatter-plot showing a high correlation between the actual and predicted values.

We can use a QQ plot to see whether our predictions are more or less of the same quality across the entire range of revenue. The QQ plot shows that the model predicts revenue quite well accross the range, although revenue in the lower range is slightly underpredicted.

```{r}
# Creating a QQ plot with base R
qqplot(test$Revenue.log, pred, main = "QQ Plot of Revenue (log)", xlab = "Quantiles of Actual Revenue", ylab = "Quantiles of Predicted Revenue")
abline(0, 1, col = "red")  # Adding a 45-degree reference line
```

In summary we can say that the SVM model predicts revenue quite well, with an R-squared of 0.73 on the test set. The most important predictors are the number votes (positive and negative), and the peak number of concurrent users. This is hardly surprising, since the number of game owners is highly correlated both with these predictors and revenue. Therefore, this model may be of limited value as a revenue predictor, since the number of votes and peak concurrent users is probably not known before the game is widely sold, at which point the revenue is already known. Further investigation is needed to determine whether these predictors are leading indicators of revenue, which may well be the case.
...
test_prob <- predict(indie_svm, newdata = test, type = "prob")

# Extract probabilities for the positive class (assuming it's the second column)
prob_positive_class <- test_prob[,2]

# Generate ROC curve
roc_curve <- roc(test$Genre.Indie, prob_positive_class)
